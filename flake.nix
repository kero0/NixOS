{
  ...
  outputs = inputs@{ self, nixpkgs, nix, nixos-hardware, darwin, home-manager
    , agenix, git-hooks, ... }:
    let
      mpkgs = system:
        import nixpkgs {
          ...
          overlays = [ inputs.nixgl.overlay ] ++ (let path = ./overlays;
          in with builtins;
      ...
      mHMmodules = hostname: myuser: pkgs:
        (umport {
    ...
    in {
      ...
      darwinConfigurations."Kirolss-MacBook-Pro" = let
      ...
      in (f "aarch64-darwin" // f "x86_64-linux");
      devShell = let
        f = system: {
