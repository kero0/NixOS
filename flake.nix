{
  ...
  outputs = inputs@{ nixpkgs, nix, nixos-hardware, home-manager, agenix
                   , sops-nix, ... }:
                     let
                       mpkgs = system:
                         import nixpkgs {
                           ...
                           overlays = [ ] ++ (let path = ./packages/overlays;
                                              in with builtins;
                       ...
                       mmodules = { hostname, myuser, pkgs, homedir ? mhomedir myuser pkgs
                                  , inc_gui ? pkgs.stdenv.isLinux, additional_configs ? [ ]
                                  , basic_config ? [ ./configuration.nix ./packages ./fonts.nix ]
                                                   ++ nixpkgs.lib.lists.optional pkgs.stdenv.isLinux {
                                                     system.stateVersion = stateVersion;
                                                   }, use_hm ? true, hm_basic ? [
                                                     ./home-manager
                                                     ...
                                                     ++ nixpkgs.lib.lists.optional inc_gui ./linux-basic-gui ++ [
                                                       agenix.${
    ...
    in {
      devShells = nixpkgs.lib.foldr nixpkgs.lib.mergeAttrs { } (map (system: {
      ...
      darwinConfigurations."Kirolss-MacBook-Air" = let
